...
export async function DownloadInstruments() {
  const routes = ["https://v2api.tradesmartonline.in/NSE_symbols.txt.zip", "https://v2api.tradesmartonline.in/NFO_symbols.txt.zip", "https://v2api.tradesmartonline.in/CDS_symbols.txt.zip", "https://v2api.tradesmartonline.in/MCX_symbols.txt.zip", "https://v2api.tradesmartonline.in/BSE_symbols.txt.zip"];
...
export function GetOptionStrike(params, fd) {
  try {
    ...
    const filteredInstruments = fileData.filter((inst) => inst.Exchange === Exchange && inst.Symbol === Symbol && inst.Instrument === Instrument && inst.Expiry === Expiry);
